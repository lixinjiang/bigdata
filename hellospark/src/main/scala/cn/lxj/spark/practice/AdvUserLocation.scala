...
object AdvUserLocation {
  def main(args: Array[String]): Unit = {
    ...
    //本地几个线程运行，*表示尽可能多的线程运行
    val sc = new SparkContext(conf)
    val rdd0 = sc.textFile("E:\\IdeaProjects\\bigdata\\hellospark\\src\\main\\resources\\bs_log").map(line => {
    ...
    val rdd2 = sc.textFile("E:\\IdeaProjects\\bigdata\\hellospark\\src\\main\\resources\\lac_info.txt").map(line => {
      val f = line.split(",")
      //(基站ID， （经度，纬度）)
      (f(0), (f(1), f(2)))
    })
    ...
    rdd5.saveAsTextFile("E:\\IdeaProjects\\bigdata\\hellospark\\out\\out2")
    sc.stop()
  }
}
