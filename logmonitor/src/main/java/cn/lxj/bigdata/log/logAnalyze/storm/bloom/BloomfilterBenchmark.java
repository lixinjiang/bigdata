...
public class BloomfilterBenchmark {
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        for (int i = 0; i < elementCount; i++) {
            byte[] b = new byte[200];
            r.nextBytes(b);
            existingElements.add(new String(b,"UTF-8"));
        }
        //创建500000个元素，用作比较
        ...
        for (int i = 0; i < elementCount; i++) {
            ...
            nonExistingElements.add(new String(b,"UTF-8"));
        }

        // 设置一个空的布隆过滤器，设置命中高精度和预期存放的最大元素个数据
        // 这个构造器能够，能够自动算出hash函数的次数
        BloomFilter<String> bf = new BloomFilter<>(0.001, elementCount);
        // 打印测试的元素个数
