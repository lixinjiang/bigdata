...
public class LogAnalyzeDao {
    private static Logger logger = LoggerFactory.getLogger(LogAnalyzeDao.class);
    private JdbcTemplate jdbcTemplate;

    public LogAnalyzeDao() {
        jdbcTemplate = new JdbcTemplate(DateSourceUtil.getDataSource());
    }
    ...
    public List<LogAnalyzeJobDetail> loadJobDetailList() {
        String sql = "SELECT condi.`jobId`,condi.`field`,condi.`value`,condi.`compare`  FROM `log_analyze`" +
                ".`log_analyze_job` AS job " +
                " LEFT JOIN `log_analyze`.`log_analyze_job_condition` AS condi   ON job.`jobId` = condi.`jobId` " +
                " WHERE job.`status` =1";
    ...
    public int[][] saveMinuteAppendRecord(List<BaseRecord> appendDataList) {
        String sql = "INSERT INTO `log_analyze`.`log_analyze_job_nimute_append` (`indexName`,`pv`,`uv`,`executeTime`," +
                "`createTime` ) " +
                "VALUES (?,?,?,?,?)";
    ...
    public List<BaseRecord> sumRecordValue(String startTime, String endTime) {
        ...
        return jdbcTemplate.query(sql
                , new BeanPropertyRowMapper<>(BaseRecord.class));
    }

    public static void main(String[] args) {
        System.out.println(new LogAnalyzeDao().sumRecordValue("2019-01-30 20:10:17", "2019-01-30 25:10:17"));
    }
}