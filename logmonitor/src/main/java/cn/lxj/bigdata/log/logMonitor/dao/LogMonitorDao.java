package cn.lxj.bigdata.log.logMonitor.dao;

import cn.lxj.bigdata.log.logMonitor.domain.App;
import cn.lxj.bigdata.log.logMonitor.domain.Record;
import cn.lxj.bigdata.log.logMonitor.domain.Rule;
import cn.lxj.bigdata.log.logMonitor.domain.User;
...
public class LogMonitorDao {
    ...
    public List<Rule> getRuleList() {
        String sql = "SELECT `id`,`name`,`keyword`,`isValid`,`appId` FROM `log_monitor`.`log_monitor_rule` WHERE " +
                "isValid =1";
    ...
    public List<App> getAppList() {
        String sql = "SELECT `id`,`name`,`isOnline`,`typeId`,`userId`  FROM `log_monitor`.`log_monitor_app` WHERE " +
                "isOnline =1";
    ...
    public List<User> getUserList() {
        String sql = "SELECT `id`,`name`,`mobile`,`email`,`isValid` FROM `log_monitor`.`log_monitor_user` WHERE " +
                "isValid =1";
    ...
    public void saveRecord(Record record) {
        String sql = "INSERT INTO `log_monitor`.`log_monitor_rule_record`" +
                ...
                "VALUES ( ?,?,?,?,?,?,?)";
        jdbcTemplate.update(sql, record.getAppId(), record.getRuleId(), record.getIsEmail(), record.getIsPhone(), 0,
                record.getLine(), new Date());
    }
