...
public class ParserOrderMqBolt extends BaseRichBolt{
    ...
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        // change "maxActive" -> "maxTotal" and "maxWait" -> "maxWaitMillis" in all example
        JedisPoolConfig config = new JedisPoolConfig();
        // 控制一个pool最多有多少个状态为idle（空闲的）的jedis实例
        config.setMaxIdle(5);
        // 控制一个pool可以分配多少个jedis实例，通过pool.getResource（）来获取
        // 如果赋值为-1,则表示不限制：如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted（耗尽）。
        // 在borrow一个redis实例时候，是否提前进行validate操作，如果为true，则得到的jedis实例均是可用的。
        config.setMaxTotal(1000 * 100);
        // 表示当borrow（引入）一个jedis实例的时候，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException
    ...
    public void execute(Tuple input) {
        ...
        // 整个网站，各个业务线，各个品类，各个店铺，各个品牌，各个商品
        // 获取整个网站的金额统计指标
