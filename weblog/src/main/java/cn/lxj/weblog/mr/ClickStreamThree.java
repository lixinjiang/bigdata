 ...
 * ClickStreamThree
 * description
 * 将清洗之后的日志梳理出来点击流pageviews模型数据
 * 输入数据是清洗过后的结果数据
 * 区分出每一次会话，给每一次visit（session）增加sesson-id（随机UUID）
...
public class ClickStreamThree {
    static class ClickStreamMapper extends Mapper<LongWritable, Text, Text, WebLogBean> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            v.set("true".equals(fields[0]), fields[1], fields[2], fields[3], fields[4], fields[5],
                ...
                for (int i = 0; i < beans.size(); i++) {
                    ...
                    if (timeDiff < 30 * 60 * 1000) {
                        v.set(session + "\001" + key.toString() + "\001" + beans.get(i - 1).getRemote_user() + "\001"
                                ...
                                "\001"
                                + beans.get(i - 1).getHttp_user_agent() + "\001" + beans.get(i - 1)
                                .getBody_bytes_sent() + "\001" + beans.get(i - 1).getStatus());
                        context.write(NullWritable.get(), v);
                        step++;
                    } else {
